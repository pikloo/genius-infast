...
class Genius_Infast_WooCommerce
{
	...
	private function get_or_create_document(WC_Order $order, Genius_Infast_API $api, $customer_id)
	{
		...
		$payload = $this->build_document_payload($order, $customer_id, $api);
		...
		$order->save();
		$order->add_order_note(sprintf(__('Facture %s créée sur INFast.', 'genius_infast'), $document_id));
	...
	private function ensure_payment_recorded(WC_Order $order, Genius_Infast_API $api, $document_id)
	{
		...
		$order->save();
		$order->add_order_note(__('Paiement enregistré sur INFast.', 'genius_infast'));
	...
	private function maybe_send_document_email(WC_Order $order, Genius_Infast_API $api, $document_id)
	{
		...
		$order->save();
		$order->add_order_note(__('Facture envoyée via INFast.', 'genius_infast'));
	 ...
	 * @return array
	 */
	private function build_document_payload(WC_Order $order, $customer_id, Genius_Infast_API $api)
	{
